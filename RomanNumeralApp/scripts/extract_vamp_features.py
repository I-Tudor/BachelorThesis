...
def main():
    with open(PARC_FILEPATH, 'r') as fp:
        parc = json.load(fp)
        valid_theorytab_ids = [theorytab_id for theorytab_id, theorytab in parc.items() if has_valid_tags(theorytab)]

    if not os.path.exists(FEATURES_FILEPATH):
        with h5py.File(FEATURES_FILEPATH, 'w'):
            pass
    
    with h5py.File(FEATURES_FILEPATH, 'r') as vamp_h5f:
    ...
    with h5py.File(AUDIOS_FILEPATH, 'r') as audios_h5f:
        ...
        counter = 0
        with h5py.File(FEATURES_FILEPATH, 'a') as vamp_h5f:
            for i in range(0, len(pending_theorytab_ids), BATCH_SIZE):
                try:
                    ...
                    batch_num_beats = [parc[theorytab_id]['num_beats'] for theorytab_id in batch_theorytab_ids]
